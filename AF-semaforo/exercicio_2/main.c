...
/* ---------------------------------------- */
sem_t semaforo_continente, semaforo_ilha, semaforo_controlador;
int entrou, saiu, cabeceira_atual;
...
void ponte_inicializar() {
    ...
    sem_init(&semaforo_controlador, 0, 1);
...
void ponte_entrar(veiculo_t *v) {
	...
	// ToDo: IMPLEMENTAR!
    sem_wait(&semaforo_controlador);
    entrou++;
    sem_post(&semaforo_controlador);
    if (v->cabeceira == CONTINENTE) {
        sem_wait(&semaforo_continente);
    } else if (v->cabeceira == ILHA) {
...
void ponte_sair(veiculo_t *v) {
    ...
    if (saiu == veiculos_turno) {
        ...
        fflush(stdout);
        if (cabeceira_atual == CONTINENTE) {
            cabeceira_atual = ILHA;
            for (int i = 0; i < veiculos_turno; i++) {
                sem_post(&semaforo_ilha);
            }
        } else if (cabeceira_atual == ILHA) {
            cabeceira_atual = CONTINENTE;
            for (int i = 0; i < veiculos_turno; i++) {
...
void ponte_finalizar() {
    ...
    sem_destroy(&semaforo_controlador);
